...
class Board
{
	...
	*/
	...
	{
		$list = db()->table("board")->orderBy([["boardNo", "asc"]])->rows();
	...
	*/
	public function registerComment()
	{
		...
		/** 댓글 갯수 업데이트 */
		$cnt = db()->table("boardComment")->where(["idxBoard" => $this->params['idxBoard']])->count();
		
	
		db()->table("boardData")
				->data(["commentCount" => $cnt])
				->where(["idx" => $this->params['idxBoard']])
				->update();
	...
	* @return Array 
					...
					- offset 게시글 시작 지점
	*/
	public function getList($id, $page = 1, $qs = "", $rows = "")
	{
		$page = $page?$page:1;
		$limit = $rows?$rows:20;
		$offset = ($page - 1) * $limit;
		...
		$obj = db()->table("boardData", $joinTable)
						->where($where)
						->select($columns)
						->limit($limit, $offset);
		if ($this->isBest) { // 베스트 게시글 
			$obj->orderBy([["{$px}boardData.hit + {$px}boardData.commentCount", "desc"]]);
		} else { // 일반 게시글 
			$obj->orderBy([["{$px}boardData.regDt", "desc"]]);
		}			
		...
		if ($list) {
			$file = App::load(\Component\File::class);
			foreach ($list as $k => $v) {
				$list[$k]['attachFiles'] = $file->getGroupFiles($v['gid']);
				$stamp = strtotime($v['regDt']);
				if ($stamp > time() - (60 * 60 * 5)) {
					$list[$k]['isNew'] = true;
				} else {
					$list[$k]['isNew'] = false;
				}
