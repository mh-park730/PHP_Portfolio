...
class Member
{
	...
	*/
	public function validateMemId()
	{
		...
		/* 자리수 + 문자 제한 체크 S */
		if (strlen($memId) < 6 || strlen($memId) > 20 || preg_match("/[^a-z0-9]/", $memId)) {
			throw new $exception("아이디는 6~20자리의 알파벳 소문자와 숫자로 구성해 주세요.");
		}
		/* 자리수 + 문자 제한 체크 E */
	...
	*/
	public function validatePassword()
	{
		...
		/**
			...
			3. 알파벳은 최소 1자리는 대문자 
		*/
		$msg = "비밀번호는 8~20자리의 알파벳 소문자, 숫자, 특수문자로 조합하여 만들어 주세요.";
		if (strlen($memPw) < 8 || strlen($memPw) > 20 || !preg_match("/[a-z]/", $memPw) || !preg_match("/[\d]/", $memPw) || !preg_match("/[~!@#$\^&*()]/", $memPw)) {
...
;
	...
	*/
	public function login($memId, $memPw)
	{
		...
		$_SESSION['member']['level'] = $info['level'];
